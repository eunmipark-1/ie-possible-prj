<template>
  <div class="bg-white border rounded-lg overflow-hidden">
    <div class="relative p-2/3">
      <img class="absolute h-full w-full object-cover" :src="property.imageUrl" :alt="property.imageAlt">
    </div>
    <div class="p-6">
      <div class="flex items-baseline">
        <span class="inline-block bg-teal-200 text-teak-800 text-xs px-2 py-1 rounded-full uppercase font-semibold tracking-wide">New</span>
      ...
      <div class="mt-2 flex items-center">
        <svg v-for="i in 5" :key="i" :class="i <= property.rating ? 'text-teal-500' : 'text-gray-400'" class="h-4 w-4 fill-current"
        ...
        <span class="ml-2 text-gray-600 text-sm">{{property.reviewCount}}</span>
      </div>
    </div>
  </div>
</template>
